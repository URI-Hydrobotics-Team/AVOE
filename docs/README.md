...
## Introduction
AVOE should be considered a set of libraries that will help you build control software for an AUV. At the core, AVOE provides generalized components for managing sensors and motors, and an underlying subsystem that supports networking, logging, an autonomoy framework and the tools to build a ground interface (deckbox), plus reference components in the form of "plugins" for extended functionality. While our codebase contains drivers and other code for our use cases, primarily our "Tardigrade" AUV, these drivers should not be considered core to AVOE, hence why they are in `plugins/`.
...
`core/` contains the core components of AVOE. It also acts as a working directory for your AVOE implementation as it is where you setup your AUV. Some notable files that implement core functionality include `io.h`, `log.h`, `motor.h`, `sensor.h` and `vehicle.h`. There is a build script that compiles AVOE (`build.sh`), however we are working to setup a proper build system using make. 
...
`lib/` is our primary directory for commonly used components of AVOE, the files that are currently maintained are `lib.h/.cpp` and `clock.h/.cpp`. `lib.h/.cpp` contains helper methods such as `appendstr`, `initstr`, and `vector_t` structs for data storage of 3D-4D Vector objects. `clock.h/.cpp` contains a timer implementation for semi-percise timing events. `network.h` provides core I/O for UDP network sockets. `controller.h` and `keyboard.h` provide easy to use functions for getting gamepad and keyboard input from Linux. `globals.h` stores global macros, such as AVOE version.
#### `plugins/`

`plugins/` contains components that are not part of the core of AVOE, but are needed for Navigation, Control, and Sensor/Motor drivers.

- `Controllers`
 Modules that convert 3D thrust vectors into individual thruster values based on a "thrust table".
...
- `Gamepad Maps`
Functions that map raw controller input of the type `controller_generic_raw` to `controller_generic_profile` which can be more easily utilized. (F710, Logitech) and (Six-Axis, Sony PlayStation 3) are avaliable. See `/lib/controller.h` for more info.
	
- `Math`
Currently holds an old testing version of our PID controller. It is completely Virtual for output and is not established for actual testing params. 
	
- `Middleware`
"Middleware" in this context refers to functions that bridge `sensor_t` or `motor_t` objects with their hardware driver counterparts. This data translation is required becuase `motor_t` and `sensor_t` objects store data in character arrays, so functions are needed to convert to and from numerical data types in order to store and read from these objects. See the `sensor_t` and `motor_t` sections for more information.
e
- `Mission Control`
Handles autonomous control by operation on a queue of "mission objects" that define movement. Mission Control is in charge of starting, incrementing, and stopping this queue along with invoking the necesary controller objects. Contains the two different versions of Mission Control, or MC. MC_Relative is mission control that uses only the IMU for navigation, MC_VPPN operates solely on CV. The version of Mission Control we want in the end is both of these combined into each other for max error correction. 

#### `test/`
`test/` holds various demo programs that can be used to test out different aspects of AVOE to check functionality. There is not many tests here as most changes made during the creation of AVOE were tested Physically due to time constraints and time needed to get AVOE into a zero error state. 
## Installation
...
## Sensors
AVOE has a high level of sensor abstraction, what this means is that you can create a sensor using our `sensor_t` object and pull telemetry data easily from your bot, all that is needed to implement this is a driver that can be integrated/rewritten into AVOE. A `sensor_t` object interfaces with drivers and other parts of AVOE through "middleware", examples of which can be found under `plugins/`. It is up to you on how you want your `sensor_t` objects to get data from the outside, (i.e socket, pigpio, serial device, etc.) 
...
## Motors
Motors and actuators exist in AVOE  as `motor_t` objects and operate very similar to `sensor_t`. With our current use of AVOE, we have a plugin titled `PPSTI (Pi Pico Serial Thruster Interface)` that handles conversion of vectors to PWM values which is used by our ESC's to drive thrusters. Our implementation as of writing has some manual config work needed to be done to allow for different Motor setups, but you would be able to realistically use this with little effort. 

## Networking
Networking under AVOE is implemented at the core with `/lib/network.h`, which provides a very simple interface for sending and recieving strings over standard Linux sockets. All packets are UDP multicast as we assume the link is reliable. `io.h` provides functions for transmiting and receiving `sensor_t`, `motor_t`, `vector_t`, and generic strings over sockets in a user friendly fashion.

## `vehicle_setup.h` and `main.cpp`
